---
name: odoo-provisioning
description: Odoo integration patterns for Omnilert — identity resolution, PIN rules, registration approval flow, name formatting, personal info sync, avatar sync, bank auto-fill, storage paths, POS webhook, compliance audit cron, branch resolution, company hard delete. Read this before touching any Odoo sync, webhook, registration approval, or company provisioning code.
type: reference
---
...
- Canonical partner = `res.partner`, resolved by `x_website_key` (barcode). Fallback: email.
- Employees = `hr.employee`, bound to partner via `work_contact_id`.
- `x_website_key` is the primary identity key. Employee-level `x_website_key` is legacy fallback only.

## PIN Rules

- Reuse the existing 4-digit PIN for the same `x_website_key` if one already exists.
- Only generate a new random PIN when none is found for that key.
...
When approving a registration request:
...
6. Decrypt password (stored encrypted via `utils/secureText.ts`) only at approval time — never stored in plaintext.
7. Check employee barcode collision against Odoo before assignment.
...
- `PUT /users/me/avatar` → updates website user avatar → async sync to canonical `res.partner` + all linked `hr.employee` records.
- User Management create → fill-if-empty import from Odoo `res.partner.image_1920` → upload to `Profile Pictures/{userId}` → write `users.avatar_url`. Continue with warning if import fails — never block user creation.
...
`companyStorageRoot` comes from `req.companyContext.companyStorageRoot` — format `${slug}-prod` or `${slug}-dev`. Never hardcode.
...
| Violation notice files | `{root}/Violation Notices/VN-{vnNumber}/{filename}` |

## POS Order Webhook (CSS Audit Trigger)

Route: `POST /webhooks/odoo/pos-order` — API key auth (`X-API-Key`), no JWT.

- **10% sampling**: `if (Math.random() > 0.1) return 200` silently. Only 10% of orders trigger audit creation.
- Silently skip (return 200) when `x_website_key` is empty or missing — cashier cannot be identified.

Reward tier computation from `amount_total`:
...
Runs hourly via `setInterval` initialized in `server.ts`. Queries Odoo `hr.attendance` for active records (`check_out = false`), excluding `company_id = 1`. Picks one at random, resolves company from `company_id`, inserts a `compliance` type audit into `store_audits`.

## Branch Resolution in Webhooks

Webhooks do **not** iterate per-company databases. Branch lookup is a single query:

```sql
SELECT company_id FROM branches WHERE odoo_branch_id = ?
```

Use the returned `company_id` to scope all subsequent inserts.
...
8. Delete company row — `ON DELETE CASCADE` removes all company-scoped rows atomically.
