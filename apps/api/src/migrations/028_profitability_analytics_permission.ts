...
const PERMISSION_KEY = 'analytics.view_profitability_analytics';
  ...
  await knex.transaction(async (trx) => {
    ...
    const [permission] = await trx('permissions')
      .insert({
        id: uuidv4(),
        key: PERMISSION_KEY,
        name: 'View Profitability Analytics',
        description: 'Access the Profitability Analytics page and view P&L metrics',
        category: 'analytics',
      })
      .onConflict('key')
